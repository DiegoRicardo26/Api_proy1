...
public class SetupActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        UserProfileImagen= FirebaseStorage.getInstance().getReference().child("ImagenesPerfil");
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()){
                    if (snapshot.hasChild("imagen")){
                        ...
                        Toast.makeText(SetupActivity.this, "Puedes cargar una foto...", Toast.LENGTH_SHORT).show();
                    }
                }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode==CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE){
            ...
            if (resultCode ==RESULT_OK){
                ...
                dialog.setMessage("Se está guardando tu foto de perfil");
                ...
                filePath.putFile(resultUri).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                        if (task.isSuccessful()){
                            Toast.makeText(SetupActivity.this, "Imagen se ha guardado", Toast.LENGTH_SHORT).show();
                                        ...
                                        public void onComplete(@NonNull Task<Void> task) {
                                            if (task.isSuccessful()){
                                               Picasso.get()
                                                       ...
                                                       .error(R.drawable.welcome)
                                                       .into(imagen_setup);
                                                Toast.makeText(SetupActivity.this, "Imagen Guardada en base de datos", Toast.LENGTH_SHORT).show();
                                               dialog.dismiss();

                                            }else{
                                                String error = task.getException().getMessage();
                                                Toast.makeText(SetupActivity.this, "Imagen no guardada"+error, Toast.LENGTH_SHORT).show();
                                                dialog.dismiss();
                                            }
                                        }
                                    });
                ...
                Toast.makeText(this, "La imagen no es soportada, intente de nuevo", Toast.LENGTH_SHORT).show();
                dialog.dismiss();
            }
        }
    }
    ...
    private void GuardarInformacionDB() {
        ...
        if (TextUtils.isEmpty(nom)){
            Toast.makeText(this, "Debe de ingrasar su nombre", Toast.LENGTH_SHORT).show();
        }else if (TextUtils.isEmpty(ciu)){
            Toast.makeText(this, "Debe de ingrasar su ciudad", Toast.LENGTH_SHORT).show();
        } else if (TextUtils.isEmpty(gen)){
            Toast.makeText(this, "Debe de ingrasar su genero", Toast.LENGTH_SHORT).show();
        } else if (TextUtils.isEmpty(eda)){
            Toast.makeText(this, "Debe de ingrasar su edad", Toast.LENGTH_SHORT).show();
        } else if (TextUtils.isEmpty(est)){
            Toast.makeText(this, "Debe de ingrasar su estado", Toast.LENGTH_SHORT).show();
        } else {
            dialog.setTitle("Guardando sus datos");
            dialog.setMessage("Esperar a que finalice el proceso");
            dialog.show();
            dialog.setCanceledOnTouchOutside(false);
